...
RAYGUIAPI int GuiValueBoxFloat(Rectangle bounds, const char *text, char *textValue, float *value); // Value box control for float values
...
int GuiValueBox(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    // Static variable to track the active value box
    static int *activeValueBox = NULL;

    // Temporary buffer to hold the text representation of the value
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiControlExclusiveMode)
    {
        ...
        // Check if this value box is active
        bool isActive = (activeValueBox == value);
        ...
        if (isActive)
        {
            ...
            // Only allow keys in range [48..57] (digits)
            ...
            // Finish editing on Enter or clicking outside
            if (IsKeyPressed(KEY_ENTER) || (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && !CheckCollisionPointRec(mousePoint, bounds)))
            {
                activeValueBox = NULL;
                result = 1;
            }
        ...
        {
            // If not active, check for mouse click to activate
            if (CheckCollisionPointRec(mousePoint, bounds))
            {
                state = STATE_FOCUSED;
                if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
                {
                    activeValueBox = value; // Set this box as active
                    result = 1;
                }
            }

            // Ensure value is within bounds when not active
            if (*value > maxValue) *value = maxValue;
            else if (*value < minValue) *value = minValue;
        }
    }
...
int GuiValueBoxFloat(Rectangle bounds, const char *text, char *textValue, float *value)
{
    ...
    // Variable estática para rastrear el control activo
    static void *activeControl = NULL;

    // Asignar un identificador único, por ejemplo, la dirección de 'value'
    void *currentControl = (void*)value;

    // Determinar si esta caja está en modo edición
    bool editMode = (activeControl == currentControl);
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiControlExclusiveMode)
    {
        ...
        if (editMode)
        {
            ...
            if (keyCount < RAYGUI_VALUEBOX_MAX_CHARS)
            {
                if (GetTextWidth(textValue) < bounds.width)
                {
                    ...
                    if (((key >= 48) && (key <= 57)) || // Números
                        (key == '.') || // Punto decimal
                        ((keyCount == 0) && (key == '+')) || // Signo más
                        ((keyCount == 0) && (key == '-')))   // Signo menos
                    {
                        textValue[keyCount] = (char)key;
                        keyCount++;
                        textValue[keyCount] = '\0'; // Asegurar la terminación NULA
            ...
            // Salir del modo edición al presionar Enter o hacer clic fuera
            if (IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_KP_ENTER))
            {
                activeControl = NULL;
                result = 1;
            }
            else if (!CheckCollisionPointRec(mousePoint, bounds) && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
            {
                activeControl = NULL;
                result = 1;
            }
    ...
    if (editMode)
    {
        // Nota: El texto interno de ValueBox siempre está centrado
        Rectangle cursor = {
    ...
    // Dibujar la etiqueta de texto si se proporciona
    GuiDrawText(text, textBounds,
                (GuiGetStyle(VALUEBOX, TEXT_ALIGNMENT) == TEXT_ALIGN_RIGHT) ? TEXT_ALIGN_LEFT : TEXT_ALIGN_RIGHT,
                GetColor(GuiGetStyle(LABEL, TEXT + (state * 3))));
    //--------------------------------------------------------------------
